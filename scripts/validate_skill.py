...
def validate_routes_and_regressions(runtime_only: bool) -> None:
    ...
    if skill.is_file():
        ...
        required_routes = (
            ...
            "references/practical/MBTI类型访谈与相邻类型辨析.md",
            "references/practical/MBTI沟通适配与成长训练.md",
        )
...
def validate_mbti_tool() -> None:
    tool = ROOT / "scripts/mbti_case.py"
    if not tool.is_file():
        return
    ...
    if result.returncode != 0:
        ...
        ERRORS.append(f"mbti_case.py self-test failed: {detail}")
