...
| 分不清自己的MBTI，或纠结两个相邻类型 | 用候选集、证据账本、针对性追问和反证给出带备选的工作结论，不用几句话制造假精确分数 |
| 想按人格差异改善沟通或成长 | 把四维和八功能翻译成可测试的话术、协商方法与日常能力练习，不把成长说成“换类型” |
...
```text
使用 $goutoujunshi 帮我判断更接近ENTJ还是INTJ。不要速测；请保留候选、证据、反证和能推翻结论的条件。
```

Skill会按轮提出低负担场景题，维护证据账本，专门比较最相邻的候选，并把Big Five、角色、文化和压力作为竞争解释。需要审计长案例时，可用 `python3 scripts/mbti_case.py init --help` 创建本地结构；脚本只校验调查结构，不替用户自动打出一个类型。
